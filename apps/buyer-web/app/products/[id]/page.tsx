...
function formatPrice(minor: number | string | null | undefined, currency = "KZT") {
...
export default async function ProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const media = product.sourceUrl ? mediaCatalog.entries[product.sourceUrl as keyof typeof mediaCatalog.entries] : undefined;
  const attributes = product.attributes ?? [];

  return (
    <main className={styles.page}>
      <div className={styles.shell}>
        <Link className={styles.back} href="/">← Вернуться в каталог</Link>
        <div className={styles.breadcrumbs}>Каталог / {product.category || "Стоматологические товары"}</div>
        <section className={styles.hero}>
          <div className={styles.visual}>
            {media?.securePath ? <img src={media.securePath} alt={media.altText ?? product.name} draggable={false} /> : <span>Фото товара<br />готовится</span>}
          </div>
          <div className={styles.summary}>
            <span className={styles.eyebrow}>{product.category || "Стоматологические товары"}</span>
            <h1>{product.name}</h1>
            {product.brand ? <p className={styles.brand}>{product.brand}{product.manufacturer ? ` · ${product.manufacturer}` : ""}</p> : null}
            <p className={styles.description}>{product.description || "Карточка товара DentMarket с описанием, характеристиками и предложениями поставщиков."}</p>
            <div className={styles.facts}>
              <span><strong>{product.offers.length}</strong><small>предложений</small></span>
        ...
        <section className={styles.contentGrid}>
          <div className={styles.panel}>
            ...
            {attributes.length ? <dl className={styles.attributes}>{attributes.map(([key, value]) => <div key={`${key}-${value}`}><dt>{key}</dt><dd>{value}</dd></div>)}</dl> : <p className={styles.muted}>Характеристики будут дополнены после следующей выгрузки поставщика.</p>}
