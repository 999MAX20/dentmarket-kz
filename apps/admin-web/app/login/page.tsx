...
export default function AdminLogin() {
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.card}><div className={styles.brand}>DM <span>Operations</span></div><p className={styles.eyebrow}>Защищённый контур</p><h1>{mode === "identity" ? "Вход оператора" : mode === "enroll" ? "Подключите MFA" : "Подтвердите MFA"}</h1>
      {mode === "identity" ? <><p>Используйте корпоративную учётную запись организации-оператора.</p><div ref={googleButton} className={styles.google} />{!googleClientId && <div className={styles.unavailable}>Google Client ID не настроен</div>}<button className={styles.apple} onClick={() => void apple()} disabled={busy || !appleClientId}> Продолжить с Apple</button></> : <><p>{mode === "enroll" ? "Добавьте секрет в приложение-аутентификатор, сохраните recovery-коды и введите текущий код." : "Введите код из приложения-аутентификатора или recovery-код."}</p>{enrollment && <div className={styles.secret}><code>{enrollment.secret}</code><small>{enrollment.recoveryCodes.join(" · ")}</small></div>}<form onSubmit={verify}><input name="code" placeholder="000000" autoComplete="one-time-code" required /><button disabled={busy}>{busy ? "Проверяем…" : "Продолжить"}</button></form></>}
      {error && <div className={styles.error} role="alert">{error}</div>}<footer>Social OIDC · TOTP MFA · tenant-bound JWT</footer></section>
  </main>;
}
