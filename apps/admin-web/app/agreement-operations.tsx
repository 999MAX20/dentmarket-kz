...
export function AgreementOperations() {
  ...
  return <section className={styles.panel} id="agreements"><header><div><p>Юридический контур</p><h2>Договоры, ожидающие ЭЦП</h2></div><Button appearance="secondary" onClick={() => void refresh()} disabled={loading}>Обновить</Button></header>{error ? <div className={styles.error}>{error}</div> : null}{loading ? <div className={styles.loading}><Spinner label="Проверяем очередь" /></div> : !items.length ? <div className={styles.empty}><strong>Очередь пуста</strong><span>Все договоры подписаны оператором или ещё не созданы поставщиками.</span></div> : <div className={styles.list}>{items.map((item) => <article key={item.id}><div><strong>{item.agreementNumber}</strong><span>Поставщик: {item.supplierOrganizationId}</span><small>Создан {new Intl.DateTimeFormat("ru-KZ", { dateStyle: "medium", timeStyle: "short" }).format(new Date(item.createdAt))}</small></div><div className={styles.signatures}><span data-ready={item.signing.supplierSigned}>Поставщик {item.signing.supplierSigned ? "✓" : "ожидается"}</span><span data-ready={item.signing.operatorSigned}>Оператор {item.signing.operatorSigned ? "✓" : "ожидается"}</span></div><Button appearance="primary" disabled={!item.signing.available || busy === item.id} onClick={() => void sign(item)}>{busy === item.id ? "Открываем…" : item.signing.operatorSigned ? "Подписано" : "Подписать ЭЦП"}</Button></article>)}</div>}<footer>Активация выполняется автоматически только после двух валидных подписей неизменяемого документа.</footer></section>;
}
