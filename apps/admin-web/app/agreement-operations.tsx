...
export function AgreementOperations() {
  ...
  return <section className={styles.panel} id="agreements"><header><div><p>Договоры с поставщиками</p><h2>Ожидают подписи ЭЦП</h2></div><Button appearance="secondary" onClick={() => void refresh()} disabled={loading}>Обновить</Button></header>{error ? <div className={styles.error}>{error}</div> : null}{loading ? <div className={styles.loading}><Spinner label="Проверяем очередь" /></div> : !items.length ? <div className={styles.empty}><strong>Очередь пуста</strong><span>Все созданные договоры уже подписаны оператором.</span></div> : <div className={styles.list}>{items.map((item) => <article key={item.id}><div><strong>{item.agreementNumber}</strong><span>Поставщик: {item.supplierOrganizationId}</span><small>Создан {new Intl.DateTimeFormat("ru-KZ", { dateStyle: "medium", timeStyle: "short" }).format(new Date(item.createdAt))}</small></div><div className={styles.signatures}><span data-ready={item.signing.supplierSigned}>Поставщик {item.signing.supplierSigned ? "✓" : "ожидается"}</span><span data-ready={item.signing.operatorSigned}>Оператор {item.signing.operatorSigned ? "✓" : "ожидается"}</span></div><Button appearance="primary" disabled={!item.signing.available || busy === item.id} onClick={() => void sign(item)}>{busy === item.id ? "Открываем…" : item.signing.operatorSigned ? "Подписано" : "Подписать ЭЦП"}</Button></article>)}</div>}<footer>Договор станет активным после двух действительных подписей одной версии документа.</footer></section>;
}
