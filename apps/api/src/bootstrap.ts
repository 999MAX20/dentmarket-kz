...
export async function createMarketplaceApp(options: { serverless?: boolean } = {}) {
  ...
  const openApi = new DocumentBuilder().setTitle("B2B Procurement Platform API").setDescription("Industry-independent procurement core").setVersion("0.1.0").build();
