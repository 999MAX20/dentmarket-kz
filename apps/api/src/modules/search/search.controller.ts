...
export class SearchController {
  ...
  search(
    ...
    const parsed = searchCatalogSchema.safeParse(query);
  ...
  compare(
    ...
    const parsed = compareOffersSchema.safeParse({ ...query, productId });
    if (!parsed.success) throw new BadRequestException(parsed.error.flatten());
    return this.searchService.compare(
      parsed.data,
      this.context(actorId, organizationId),
    );
  }
  ...
  @RequirePermissions("catalog.product.moderate")
  analyticsReport(@Query("days") days: string) {
    return this.analytics.report(Number(days || 30));
  }
}
...
export class PublicCatalogController {
  constructor(
    private readonly searchService: SearchService,
    private readonly analytics: SearchAnalyticsService,
    private readonly catalogRelations: CatalogService,
  ) {}
  @Get("cities")
  cities() {
    return this.searchService.publicCities();
  }
  private input(query: Record<string, unknown>) {
    const organizationId = environment().PUBLIC_CATALOG_ORGANIZATION_ID;
    return {
      query: { ...query, buyerOrganizationId: organizationId },
      context: { actorId: "public-catalog", organizationId },
    };
  }
  ...
  search(@Query() query: Record<string, unknown>) {
    ...
    const parsed = searchCatalogSchema.safeParse(input.query);
    if (!parsed.success) throw new BadRequestException(parsed.error.flatten());
  ...
  compare(
    ...
    const input = this.input({ ...query, productId });
    const parsed = compareOffersSchema.safeParse(input.query);
    if (!parsed.success) throw new BadRequestException(parsed.error.flatten());
  ...
  relations(@Param("productId") productId: string) {
    return this.catalogRelations.publicRelations(productId);
  }
}
