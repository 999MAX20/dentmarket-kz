...
const requiredFiles = [".env.production.example", "compose.production.yaml", "infra/Caddyfile", "scripts/backup.sh", "scripts/restore.sh", "scripts/backup-production.sh", "scripts/restore-production.sh", "docs/production-deployment.md", ".github/workflows/release.yml", ".github/workflows/security.yml"];
for (const file of requiredFiles) {
  const check = spawnSync("test", ["-s", file]);
  if (check.status !== 0) throw new Error(`Required production artifact is missing or empty: ${file}`);
}
...
const run = (env) => spawnSync(process.execPath, ["-e", "require('./apps/api/dist/src/platform/config/environment.js').environment(); console.log('valid')"], { env, encoding: "utf8" });
const accepted = run(valid); if (accepted.status !== 0) throw new Error(`Valid production contract rejected: ${accepted.stderr}`);
const insecure = run({ ...valid, CORS_ORIGINS: "http://localhost:3000" }); if (insecure.status === 0) throw new Error("Insecure localhost production CORS was accepted");
const mockPayment = run({ ...valid, PAYMENT_PROVIDER_MODE: "mock", PAYMENT_GATEWAY_URL: "" }); if (mockPayment.status === 0) throw new Error("Mock payment mode was accepted in production");
console.log(JSON.stringify({ productionContract: true, insecureCorsRejected: true, mockPaymentsRejected: true, artifacts: requiredFiles }, null, 2));
