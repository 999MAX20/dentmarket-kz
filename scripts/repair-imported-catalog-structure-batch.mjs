...
try {
  ...
  `, [industry.id]);
  const category = (await client.query('SELECT id FROM "Category" WHERE "industryId" = $1 AND code = $2', [industry.id, 'imported-catalog'])).rows[0];
  ...
  console.log(JSON.stringify({ variants: variants.rowCount, industries: industries.rowCount, categories: categories.rowCount }));
} catch (error) {
