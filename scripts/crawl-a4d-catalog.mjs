...
function links(html) {
  const out = new Set();
  for (const m of html.matchAll(/href=["'](\/content\/[^"'#?]+)["']/gi)) out.add(m[1]);
  return [...out];
}
function first(html, re) { const m = html.match(re); return clean(m?.[1] || ''); }
...
const rows = await pool([...productLinks], async (path) => {
  ...
  return {
    source_slug: 'allfordent', supplier_name: 'ТОО AllForDent', source_url: base + path,
    external_id: article || path.split('/').pop(), name: title,
    brand: '', category: category || 'Стоматологические материалы', unit: 'piece',
    currency: 'KZT', price: '', quantity: '', is_regulated: 'false',
    notes: 'Публичная карточка каталога AllForDent; цена и наличие требуют подтверждения поставщиком.'
  };
});

const uniq = new Map();
for (const row of rows) uniq.set(`${row.external_id}|${row.name}`, row);
const cols = ['source_slug','supplier_name','source_url','external_id','name','brand','category','unit','currency','price','quantity','is_regulated','notes'];
await fs.mkdir('data/imports', { recursive: true });
await fs.writeFile(out, [cols.join(','), ...[...uniq.values()].map((r) => cols.map((c) => esc(r[c])).join(','))].join('\n') + '\n');
console.log(JSON.stringify({ categories: categories.length, productLinks: productLinks.size, rows: uniq.size, out }));
