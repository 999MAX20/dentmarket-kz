...
Beauty подготовлен архитектурно: 12 поставщиков, 44 брендовые позиции, 12 категорий, onboarding-очередь и публичный intake UCG на 10 товарных строк. Эти Beauty-строки пока сохранены в intake-файле и ещё не записаны в БД, потому что в текущем окружении отсутствует `DATABASE_URL`.
...
- 10 товарных строк UCG с публичными изображениями и source URL;
...
- 10 UCG-строк ещё не записаны в БД;
...
В текущей рабочей сессии `DATABASE_URL` отсутствует, поэтому запись новых Beauty-карточек в БД и production-sync не выполнялись.
