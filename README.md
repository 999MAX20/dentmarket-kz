...
- compliance-аудит структуры: `PASS`, 0 блокирующих нарушений в белом списке, 7 781 review-флагов для последующего обогащения; отчёт: `data/reports/catalog-compliance-report.md`;
- полностью готовых к продаже карточек по строгому критерию «карточка + точное фото + описание + вариант + supplier offer + цена + остаток + подтверждённый поставщик» сейчас **0**;
- `approved canonical` означает разрешённую canonical-заготовку для витрины и быстрого matching, а не наличие товара для заказа;
- публичный fallback содержит 19 172 карточки, но значительная часть offers создана как пустой коммерческий слот: без цены, остатка и подтверждения поставщика.
